...
def test_cache_key_salting():
    """
    This tests that cache salts are applied during hashing.
    """
    manager = KVCacheManager(
        make_kv_cache_config(16, 11),
        max_model_len=8192,
    ...
    assert block_hashes[1].extra_keys == None
    assert block_hashes[2].extra_keys == None
    ...
    # The just completed block should not have hashes with extra keys.
    assert len(block_hashes) == 4
    assert block_hashes[3].extra_keys == None

    # Cache hit with a new request that has the same salt.
    ...
    # Cache failure with a new request that has a different same salt.
    token_ids = common_token_ids + [4] * 11
    req2 = make_request("2",
                        token_ids,
                        cache_salt="salt2")
